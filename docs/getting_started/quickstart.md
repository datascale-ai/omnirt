...
- `torch_npu` 的 `graph_mode` 对部分算子仍会失败，OmniRT 会自动把失败模块回退到 eager 并在 `RunReport.backend_timeline` 里记录（见 [架构说明](../developer_guide/architecture.md)）。
...
GitHub Pages 发布说明见 [文档发布](../community/publishing_docs.md)。
