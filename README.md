...
<p align="center">
  <strong>面向 CUDA 与 Ascend 后端的统一图像 / 视频 / 语音 / 数字人生成运行时</strong>
</p>
...
OmniRT 是一个开源的多模态生成运行时，把 **文本→图像 / 图像→图像 / 文本→语音 / 文本→视频 / 图像→视频 / 音频→数字人** 等任务统一到同一套请求契约、同一组 CLI / Python API / HTTP 服务，以及可替换的硬件后端之上。切换不同模型家族时，你**不需要**重新适应一整套新的运行时接口。
...
- **16+ 模型家族** — SD1.5 / SDXL / SD3 / FLUX / FLUX2 / WAN / SVD / AnimateDiff / ChronoEdit / CosyVoice / FlashTalk / FlashHead 等
- **产物标准化** — 图像统一导出为 PNG，语音统一导出为 WAV，视频统一导出为 MP4，每次运行都会生成一份 `RunReport`
...
- `cosyvoice3-triton-trtllm` 已通过官方 Triton / TensorRT-LLM 路线完成 CUDA 真机验证；26 条 streaming benchmark: `RTF=0.1303`，平均首包 `699.13ms`
...
- **用户指南**
  ...
  - 文本到音频:[docs/user_guide/generation/text2audio.md](./docs/user_guide/generation/text2audio.md)
...
- **开发者指南**
  ...
  - CosyVoice Benchmark:[docs/developer_guide/cosyvoice_benchmark.md](./docs/developer_guide/cosyvoice_benchmark.md)
